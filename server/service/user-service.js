...
class UserService{  //вся логика прописывается в сервисах // этот sсервис для работы с пользователем
    async registration(email, password){
        ...
        const user = await UserModel.create({email, password: hashPassword, activationLink}); //сохраняем пользователя в бд
        await mailService.sendActivationMail(email,`${process.env.API_URL}/api/activate/${activationLink}`); //отправляется письмо для активации, и ссылку на эндпойнт, он переходит и акк активируется
    ...
    async login(email, password){
        ...
        const isPassEquals = await bcrypt.compare(password, user.password) //второй пароль, это пароль из бд
        if(!isPassEquals){
